#!/usr/bin/env python3
"""
Fixed Demo Data Creation Script - Creates comprehensive data for ALL app features
"""
...
def create_fixed_demo_data():
    """Create comprehensive demo data showcasing ALL app features"""
    print("🎬 Creating FIXED Demo Data for ALL App Features...")
    ...
    try:
        ...
        print("❌ Demo user not found. Please run create_enhanced_demo.py first.")
        return
    
    # Create additional comprehensive posts with enhanced image analysis
    additional_posts = [
        {
            'platform': 'instagram',
            ...
            'image_analysis': json.dumps({
                ...
                'objects': ['bamboo', 'forest', 'path', 'nature', 'sunlight', 'zen', 'walkway'],
                'faces': 0,
                'aesthetics': {'score': 0.95, 'composition': 'exceptional', 'lighting': 'ethereal', 'depth': 'excellent'},
        ...
        },
        {
            ...
            'image_analysis': json.dumps({
                ...
                'objects': ['person', 'suit', 'office', 'professional', 'corporate', 'business'],
                'faces': 1,
                ...
                'quality_score': 0.45,
                'spam_indicators': ['urgent', 'caps_lock', 'excessive_exclamation']
            }),
        ...
        },
        {
            ...
            'image_analysis': json.dumps({
                ...
                'objects': ['people', 'park', 'volunteers', 'cleanup', 'community', 'families', 'children', 'seniors'],
                'faces': 8,
                ...
                'quality_score': 0.78,
                'authenticity_indicators': ['candid', 'real_people', 'genuine_activity']
            }),
        ...
        },
        {
            ...
            'image_analysis': json.dumps({
                ...
                'objects': ['art', 'gallery', 'people', 'paintings', 'exhibition', 'culture', 'artwork', 'visitors'],
                'faces': 12,
        ...
        },
        {
            ...
            'image_analysis': json.dumps({
                ...
                'objects': ['office', 'computers', 'team', 'startup', 'technology', 'workspace', 'monitors', 'collaboration'],
                'faces': 3,
    ...
    for i, post_data in enumerate(additional_posts):
        ...
        if SocialPost.objects.filter(user=demo_user, content_hash=content_hash).exists():
            print(f"   ⏭️  Post {i+6}: {post_data['original_user']} already exists - skipping")
            continue
    ...
    ml_models = [
        ...
        },
        {
            ...
            'model_type': 'topic',  # Using existing model type
            'version': '1.0.0',
    ...
    # Create ML prediction logs with correct field names
    print("🔮 Creating ML prediction logs...")
    posts = SocialPost.objects.filter(user=demo_user)
    authenticity_model = MLModel.objects.filter(name='Authenticity Classifier v2.1').first()
    ...
    if authenticity_model:
        for post in posts:
            MLPredictionLog.objects.get_or_create(
                ...
                defaults={
                    'prediction_value': post.authenticity_score or random.uniform(0.1, 0.9),
                    'confidence': random.uniform(0.7, 0.95),
                    'raw_output': {
                        'score': post.authenticity_score,
                        'confidence': random.uniform(0.7, 0.95),
                        'features': ['content_length', 'sentiment', 'engagement'],
    ...
    print("✅ FIXED demo environment created successfully!")
    print("\n" + "="*60)
    ...
    print(f"ML Prediction Logs: {MLPredictionLog.objects.filter(post__user=demo_user).count()}")
    ...
    print("✅ Behavior Tracking: 15+ different user interactions logged")
    ...
    print("✅ Quality Metrics: Instagram and professional scores")
    print("✅ Spam Detection: Spam indicators for low-quality content")
    
    print("\n🧠 ML INSIGHTS READY:")
    print("✅ Model Performance: 4 different ML models with metrics")
    print("✅ Prediction Logs: Individual prediction tracking")
    print("✅ Accuracy Metrics: Real performance data (76%-89%)")
    print("✅ Processing Times: Realistic timing data")
    print("✅ Feature Analysis: Detailed model parameters")
    print("✅ Confidence Scores: Prediction confidence levels")
    
    print("\n🌐 DEMO READY - ALL FEATURES POPULATED:")
